...
CREATE TABLE plantsTable (
     ...
     lon DOUBLE,
     country STRING
) WITH (KAFKA_TOPIC='grid-plants', VALUE_FORMAT = 'JSON');
...
CREATE TABLE linkTable (
     ...
     asGeoJSON STRING
) WITH (KAFKA_TOPIC='grid-static-links', VALUE_FORMAT = 'JSON');

CREATE STREAM gridLinkFlowDataStream (
     rowkey VARCHAR KEY,
     ts BIGINT,
     flow DOUBLE
) WITH (KAFKA_TOPIC='grid-link-flow-data', VALUE_FORMAT = 'JSON');
